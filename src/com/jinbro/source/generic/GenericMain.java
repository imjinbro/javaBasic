...
/*
    ...
    [와일드카드]
    - 무엇이라도 들어가도된다 : 모른다, 몰라도된다, 무엇이 들어올지 알 필요없다
    - 뭔지는 모르겠지만 타입이 설정되면 그렇게 사용하겠다 정도
    => 데이터에 관심가지는 것이 아니라 그 타입이 가지는 메서드에 관심을 가짐
    => 혹은 데이터를 저장하는 자료구조의 메서드에 관심을 가짐
    => 결론적으로 조금 더 좁은 의미로 사용하겠다는 것
    => 있는 그대로에서 무엇을 할 때 : 가진 메서드를 가지고
